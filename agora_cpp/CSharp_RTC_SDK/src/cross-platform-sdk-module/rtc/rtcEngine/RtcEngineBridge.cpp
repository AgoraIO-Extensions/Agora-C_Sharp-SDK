...
namespace common {
    ...
    int
    RtcEngineBridge::callApi(API_TYPE apiType, const std::string& parameters)
    {
        ...
        case JOIN_CHANNEL_WITH_USER_ACCOUNT: {
            ...
            ret = joinChannelWithUserAccount(token.c_str(),
                channelId.c_str(),
                userAccount.c_str());
        } break;
        ...
        case SET_VIDEO_ENCODER_CONFIGURATION: {
            ...
            json_to_VideoDimension(dimensions, videoEncoderConfiguration.dimensions, ret);
            CHECK_RET_ERROR(ret)

            get_parameter_int(config, "minFrameRate", videoEncoderConfiguration.minFrameRate, ret);
            CHECK_RET_ERROR(ret)

            get_parameter_int(config, "bitrate", videoEncoderConfiguration.bitrate, ret);
            CHECK_RET_ERROR(ret)

            get_parameter_int(config, "minBitrate", videoEncoderConfiguration.minBitrate, ret);
            CHECK_RET_ERROR(ret)
            ...
            get_parameter_int(config, "degradationPreference", degradationPreference, ret);
            CHECK_RET_ERROR(ret)
        ...
        case SET_CAMERA_CAPTURER_CONFIGURATION: {
            ...
            json_to_CameraCapturerConfiguration(config, cameraCapturerConfiguration, ret);
            CHECK_RET_ERROR(ret)
        ...
        case SET_AUDIO_PROFILE: {
            ...
            ret = setAudioProfile(AUDIO_PROFILE_TYPE(profile), AUDIO_SCENARIO_TYPE(scenario));
        } break;
        ...
        case START_AUDIO_RECORDING: {
            ...
            ret = startAudioRecording(filePath.c_str(), AUDIO_RECORDING_QUALITY_TYPE(quality));
        } break;
        ...
        case SET_AUDIO_SESSION_OPERATION_RESTRICTION: {
            ...
            ret = setAudioSessionOperationRestriction(rtc::AUDIO_SESSION_OPERATION_RESTRICTION(restriction));
#else
            ret = ERROR_CODE::ERROR_NOT_SUPPORT_API;
#endif
        } break;
        ...
        case START_SCREEN_CAPTURE_BY_DISPLAY_ID: {
            ...
            json_to_ScreenCaptureParameters(captureParams, screenCaptureParameters, videoDimensions, windowList, ret);
            ret = startScreenCaptureByDisplayId(displayId, rectangle, screenCaptureParameters);
#else
            ret = ERROR_CODE::ERROR_NOT_SUPPORT_API;
#endif
        } break;
        ...
        case START_SCREEN_CAPTURE_BY_SCREEN_RECT: {
            ...
            startScreenCaptureByScreenRect(screenRect, regionRect, screenCaptureParameters);
#else
            ret = ERROR_CODE::ERROR_NOT_SUPPORT_API;
#endif
        } break;
        ...
        case START_SCREEN_CAPTURE_BY_WINDOW_ID: {
            ...
            ret = startScreenCaptureByWindowId(reinterpret_cast<view_t>(windowId), regionRect, screenCaptureParameters);
        ...
        case SET_LIVE_TRANSCODING: {
            ...
            TranscodingUser* transcodingUser = new TranscodingUser[transcoding.userCount];
            Value val;
        ...
        case ADD_VIDEO_WATER_MARK_2: {
            ...
            json_to_Rectangle(positionInLandscapeMode, rectangle_positionInLandscapeMode, ret);
            CHECK_RET_ERROR(ret);

            watermarkOptions.positionInLandscapeMode = rectangle_positionInLandscapeMode;
            ...
            json_to_Rectangle(positionInPortraitMode, rectangle_positionInPortraitMode, ret);
            CHECK_RET_ERROR(ret);
        ...
        case SEND_CUSTOM_REPORT_MESSAGE: {
            ...
            ret = sendCustomReportMessage(id.c_str(), category.c_str(), event.c_str(), label.c_str(), value);
        } break;
    ...
    int
    RtcEngineBridge::callApi_audioEffect(API_TYPE_AUDIO_EFFECT apiType,
        const std::string& parameters)
    {
        ...
        case PLAY_EFFECT: {
            ...
            ret = playEffect(soundId, filePath.c_str(), loopCount, pitch, pan, gain, publish);
        } break;
        ...
        case SET_LOCAL_VOICE_EQUALIZATION: {
            int bandFrequency;
            get_parameter_int(document, "bandFrequency", bandFrequency,
                ret);
            CHECK_RET_ERROR(ret)
        ...
        case SET_RECORDING_AUDIO_FRAME_PARAMETERS: {
            ...
            ret = setRecordingAudioFrameParameters(sampleRate,
                channel,
        ...
        case SET_PLAYBACK_AUDIO_FRAME_PARAMETERS: {
            ...
            ret = setPlaybackAudioFrameParameters(sampleRate,
                channel,
                RAW_AUDIO_FRAME_OP_MODE_TYPE(mode),
                samplesPerCall);
        } break;
    ...
    const char*
    RtcEngineBridge::callApi_str(API_TYPE apiType, const std::string& parameters)
    {
        int ret = ERROR_CODE::ERROR_OK;
        Document document;
        document.Parse(parameters.c_str());
    ...
    int
    RtcEngineBridge::callApi(API_TYPE apiType, const std::string& parameters, void* ptr)
    {
        int ret = ERROR_CODE::ERROR_OK;
        Document document;
        document.Parse(parameters.c_str());
    ...
    int
    RtcEngineBridge::initEventHandler(IRtcEngineEventHandler* eventHandler)
    {
    ...
    int
    RtcEngineBridge::getUserInfoByUid(rtc::uid_t uid, UserInfo* userInfo)
    {
    ...
    int
    RtcEngineBridge::getUserInfoByUserAccount(const char* userAccount,
        UserInfo* userInfo)
    {
    ...
    int
    RtcEngineBridge::initialize(const char* appId, void* context, unsigned int areaCode)
    {
        //mRtcEngineEventHandler = new RtcEngineEventHandler();
        ...
        rtcEngine3->setAppType((AppType)12);
        return ret;
    }

    const char*
    RtcEngineBridge::getVersion()
    {
    ...
    int
    RtcEngineBridge::setChannelProfile(CHANNEL_PROFILE_TYPE profile)
    {
    ...
    int
    RtcEngineBridge::setClientRole(CLIENT_ROLE_TYPE role)
    {
    ...
    int
    RtcEngineBridge::joinChannel(const char* token,
        const char* channelId,
        const char* info,
        rtc::uid_t uid)
    {
        LOG_JSON(JOIN_CHANNEL, "token", token, "channelId", channelId, "info", info, "uid", uid);
        return mRtcEngine->joinChannel(token, channelId, info, uid);
    }

    int
    RtcEngineBridge::switchChannel(const char* token, const char* channelId)
    {
    ...
    int
    RtcEngineBridge::renewToken(const char* token)
    {
    ...
    int
    RtcEngineBridge::registerLocalUserAccount(const char* appId,
        const char* userAccount)
    {
        LOG_JSON(REGISTER_LOCAL_USER_ACCOUNT, "appId", appId, "userAccount", userAccount);
        return mRtcEngine->registerLocalUserAccount(appId, userAccount);
    }

    int
    RtcEngineBridge::joinChannelWithUserAccount(const char* token,
        const char* channelId,
        const char* userAccount)
    {
        LOG_JSON(JOIN_CHANNEL_WITH_USER_ACCOUNT, "token", token, "channelId", channelId, "userAccount", userAccount);
        return mRtcEngine->joinChannelWithUserAccount(token, channelId, userAccount);
    }

    const char*
    RtcEngineBridge::getErrorDescription(int code)
    {
    ...
    int
    RtcEngineBridge::setLogFilter(unsigned int filter)
    {
    ...
    int
    RtcEngineBridge::setLogFile(const char* file)
    {
    ...
    int
    RtcEngineBridge::setLogFileSize(unsigned int fileSizeInKBytes)
    {
    ...
    int
    RtcEngineBridge::setLocalRenderMode(RENDER_MODE_TYPE renderMode)
    {
    ...
    RtcEngineBridge::setLocalRenderMode(RENDER_MODE_TYPE renderMode,
        VIDEO_MIRROR_MODE_TYPE mirrorMode)
    {
        LOG_JSON(SET_LOCAL_RENDER_MODE_2, "renderMode", renderMode, "mirrorMode", mirrorMode);
        return mRtcEngine->setLocalRenderMode(renderMode, mirrorMode);
    }

    int
    RtcEngineBridge::setRemoteRenderMode(rtc::uid_t userId, RENDER_MODE_TYPE renderMode)
    {
    ...
    int
    RtcEngineBridge::setRemoteRenderMode(rtc::uid_t userId,
        RENDER_MODE_TYPE renderMode,
        VIDEO_MIRROR_MODE_TYPE mirrorMode)
    {
        LOG_JSON(SET_REMOTE_RENDER_MODE_2, "userId", userId, "renderMode", renderMode, "mirrorMode", mirrorMode);
        return mRtcEngine->setRemoteRenderMode(userId, renderMode, mirrorMode);
    }

    int
    RtcEngineBridge::enableLocalAudio(bool enabled)
    {
    ...
    int
    RtcEngineBridge::setAudioProfile(AUDIO_PROFILE_TYPE profile,
    ...
    int
    RtcEngineBridge::setDefaultMuteAllRemoteAudioStreams(bool mute)
    {
    ...
    int
    RtcEngineBridge::setPlaybackDeviceVolume(int volume)
    {
    ...
    int
    RtcEngineBridge::enableAudioVolumeIndication(int interval, int smooth, bool report_vad)
    {
        LOG_JSON(ENABLE_AUDIO_VOLUME_INDICATION, "interval", interval, "smooth", smooth, "report_vad", report_vad);
        return mRtcEngine->enableAudioVolumeIndication(interval, smooth, report_vad);
    }

    int
    RtcEngineBridge::startAudioRecording(const char* filePath,
        AUDIO_RECORDING_QUALITY_TYPE quality)
    {
        RtcEngineParameters rtcEngineParameters(mRtcEngine);
    ...
    int
    RtcEngineBridge::stopAudioRecording()
    {
    ...
    int
    RtcEngineBridge::startEchoTest(int intervalInSeconds)
    {
    ...
    int
    RtcEngineBridge::setVideoProfile(VIDEO_PROFILE_TYPE profile, bool swapWidthAndHeight)
    {
        LOG_JSON(SET_VIDEO_PROFILE, "profile", profile, "swapWidthAndHeight", swapWidthAndHeight);
        return mRtcEngine->setVideoProfile(profile, swapWidthAndHeight);
    }

    int
    RtcEngineBridge::setVideoEncoderConfiguration(const VideoEncoderConfiguration& config)
    {
    ...
    int
    RtcEngineBridge::setCameraCapturerConfiguration(const CameraCapturerConfiguration& config)
    {
    ...
    int
    RtcEngineBridge::setRemoteUserPriority(rtc::uid_t uid, PRIORITY_TYPE userPriority)
    {
    ...
    int
    RtcEngineBridge::muteLocalAudioStream(bool mute)
    {
    ...
    int
    RtcEngineBridge::muteAllRemoteAudioStreams(bool mute)
    {
    ...
    int
    RtcEngineBridge::adjustUserPlaybackSignalVolume(unsigned int uid, int volume)
    {
    ...
    int
    RtcEngineBridge::muteRemoteAudioStream(rtc::uid_t userId, bool mute)
    {
    ...
    int
    RtcEngineBridge::muteLocalVideoStream(bool mute)
    {
    ...
    int
    RtcEngineBridge::enableLocalVideo(bool enabled)
    {
    ...
    int
    RtcEngineBridge::muteAllRemoteVideoStreams(bool mute)
    {
    ...
    int
    RtcEngineBridge::setDefaultMuteAllRemoteVideoStreams(bool mute)
    {
    ...
    int
    RtcEngineBridge::muteRemoteVideoStream(rtc::uid_t userId, bool mute)
    {
    ...
    int
    RtcEngineBridge::setRemoteVideoStreamType(rtc::uid_t userId, REMOTE_VIDEO_STREAM_TYPE streamType)
    {
        LOG_JSON(SET_REMOTE_VIDEO_STREAM_TYPE, "userId", userId, "streamType", streamType);
        return mRtcEngine->setRemoteVideoStreamType(userId, streamType);
    }

    int
    RtcEngineBridge::setRemoteDefaultVideoStreamType(REMOTE_VIDEO_STREAM_TYPE streamType)
    {
    ...
    RtcEngineBridge::startAudioRecording(const char* filePath,
        ...
        AUDIO_RECORDING_QUALITY_TYPE quality)
    {
        LOG_JSON(START_AUDIO_RECORDING2, "filePath", filePath, "sampleRate", sampleRate, "quality", quality);
        return mRtcEngine->startAudioRecording(filePath, sampleRate, quality);
    }

    int
    RtcEngineBridge::startAudioMixing(const char* filePath,
        bool loopback,
        bool replace,
        int cycle)
    {
        LOG_JSON(START_AUDIO_MIXING, "filePath", filePath, "loopback", loopback, "replace", replace, "cycle", cycle);
        return mRtcEngine->startAudioMixing(filePath, loopback, replace, cycle);
    }

    int
    RtcEngineBridge::stopAudioMixing()
    {
    ...
    int
    RtcEngineBridge::pauseAudioMixing()
    {
    ...
    int
    RtcEngineBridge::resumeAudioMixing()
    {
    ...
    int
    RtcEngineBridge::setHighQualityAudioParameters(bool fullband, bool stereo, bool fullBitrate)
    {
        LOG_JSON(SET_HIGH_QUALITY_AUDIO_PARAMETERS, "fullband", fullband, "stereo", stereo, "fullBitrate", fullBitrate);
        return mRtcEngine->setHighQualityAudioParameters(fullband, stereo, fullBitrate);
    }

    int
    RtcEngineBridge::adjustAudioMixingVolume(int volume)
    {
    ...
    int
    RtcEngineBridge::adjustAudioMixingPlayoutVolume(int volume)
    {
    ...
    int
    RtcEngineBridge::getAudioMixingPlayoutVolume()
    {
    ...
    int
    RtcEngineBridge::adjustAudioMixingPublishVolume(int volume)
    {
    ...
    int
    RtcEngineBridge::getAudioMixingPublishVolume()
    {
    ...
    int
    RtcEngineBridge::getAudioMixingDuration()
    {
    ...
    int
    RtcEngineBridge::getAudioMixingCurrentPosition()
    {
    ...
    int
    RtcEngineBridge::setAudioMixingPosition(int pos /*in ms*/)
    {
    ...
    int
    RtcEngineBridge::setAudioMixingPitch(int pitch)
    {
    ...
    int
    RtcEngineBridge::getEffectsVolume()
    {
    ...
    int
    RtcEngineBridge::setEffectsVolume(int volume)
    {
    ...
    int
    RtcEngineBridge::setVolumeOfEffect(int soundId, int volume)
    {
...
#if defined(__ANDROID__) || (defined(__APPLE__) && TARGET_OS_IOS)
    int
    RtcEngineBridge::enableFaceDetection(bool enable)
    {
    ...
    int
    RtcEngineBridge::playEffect(int soundId,
        const char* filePath,
        int loopCount,
        double pitch,
        double pan,
        ...
        bool publish)
    {
        LOG_JSON(PLAY_EFFECT, "soundId", soundId, "filePath", filePath, "loopCount", loopCount, "pitch", pitch, "pan", pan, "gain", gain, "publish", publish);
        return mRtcEngine->playEffect(soundId, filePath, loopCount, pitch, pan, gain, publish);
    }

    int
    RtcEngineBridge::stopEffect(int soundId)
    {
    ...
    int
    RtcEngineBridge::stopAllEffects()
    {
    ...
    int
    RtcEngineBridge::preloadEffect(int soundId, const char* filePath)
    {
    ...
    int
    RtcEngineBridge::unloadEffect(int soundId)
    {
    ...
    int
    RtcEngineBridge::pauseEffect(int soundId)
    {
    ...
    int
    RtcEngineBridge::pauseAllEffects()
    {
    ...
    int
    RtcEngineBridge::resumeEffect(int soundId)
    {
    ...
    int
    RtcEngineBridge::resumeAllEffects()
    {
    ...
    int
    RtcEngineBridge::enableSoundPositionIndication(bool enabled)
    {
    ...
    int
    RtcEngineBridge::setRemoteVoicePosition(rtc::uid_t uid, double pan, double gain)
    {
    ...
    int
    RtcEngineBridge::setLocalVoicePitch(double pitch)
    {
    ...
    int
    RtcEngineBridge::setLocalVoiceEqualization(
        AUDIO_EQUALIZATION_BAND_FREQUENCY bandFrequency, int bandGain)
    {
        LOG_JSON(SET_LOCAL_VOICE_EQUALIZATION, "bandFrequency", bandFrequency, "bandGain", bandGain);
        return mRtcEngine->setLocalVoiceEqualization(bandFrequency, bandGain);
    }

    int
    RtcEngineBridge::setLocalVoiceReverb(AUDIO_REVERB_TYPE reverbKey, int value)
    {
    ...
    int
    RtcEngineBridge::setLocalVoiceChanger(VOICE_CHANGER_PRESET voiceChanger)
    {
    ...
    int
    RtcEngineBridge::setLocalVoiceReverbPreset(AUDIO_REVERB_PRESET reverbPreset)
    {
    ...
    int
    RtcEngineBridge::adjustRecordingSignalVolume(int volume)
    {
    ...
    int
    RtcEngineBridge::adjustPlaybackSignalVolume(int volume)
    {
    ...
    int
    RtcEngineBridge::setExternalAudioSource(bool enabled, int sampleRate, int channels)
    {
        LOG_JSON(SET_EXTERNAL_AUDIO_SOURCE, "enabled", enabled, "sampleRate", sampleRate, "channels", channels);
        return mRtcEngine->setExternalAudioSource(enabled, sampleRate, channels);
    }

    int
    RtcEngineBridge::setExternalAudioSink(bool enabled, int sampleRate, int channels)
    {
        LOG_JSON(SET_EXTERNAL_AUDIO_SINK, "enabled", enabled, "sampleRate", sampleRate, "channels", channels);
        return mRtcEngine->setExternalAudioSink(enabled, sampleRate, channels);
    }

    int
    RtcEngineBridge::setRecordingAudioFrameParameters(int sampleRate,
        int channel,
        RAW_AUDIO_FRAME_OP_MODE_TYPE mode,
        int samplesPerCall)
    {
        LOG_JSON(SET_RECORDING_AUDIO_FRAME_PARAMETERS, "sampleRate", sampleRate, "channel", channel, "mode", mode, "samplesPerCall", samplesPerCall);
        return mRtcEngine
            ->setRecordingAudioFrameParameters(sampleRate, channel, mode, samplesPerCall);
    }

    int
    RtcEngineBridge::setPlaybackAudioFrameParameters(int sampleRate,
        int channel,
        RAW_AUDIO_FRAME_OP_MODE_TYPE mode,
        int samplesPerCall)
    {
        LOG_JSON(SET_PLAYBACK_AUDIO_FRAME_PARAMETERS, "sampleRate", sampleRate, "channel", channel, "mode", mode, "samplesPerCall", samplesPerCall);
        return mRtcEngine
            ->setPlaybackAudioFrameParameters(sampleRate, channel, mode, samplesPerCall);
    }

    int
    RtcEngineBridge::setMixedAudioFrameParameters(int sampleRate, int samplesPerCall)
    {
        LOG_JSON(SET_MIXED_AUDIO_FRAME_PARAMETERS, "sampleRate", sampleRate, "samplesPerCall", samplesPerCall);
        return mRtcEngine->setMixedAudioFrameParameters(sampleRate, samplesPerCall);
    }

    int
    RtcEngineBridge::enableWebSdkInteroperability(bool enabled)
    {
    ...
    int
    RtcEngineBridge::setVideoQualityParameters(bool preferFrameRateOverImageQuality)
    {
        LOG_JSON(SET_VIDEO_QUALITY_PARAMETERS, "preferFrameRateOverImageQuality", preferFrameRateOverImageQuality);
        return mRtcEngine->setVideoQualityParameters(preferFrameRateOverImageQuality);
    }

    int
    RtcEngineBridge::setLocalPublishFallbackOption(STREAM_FALLBACK_OPTIONS option)
    {
    ...
    int
    RtcEngineBridge::setRemoteSubscribeFallbackOption(STREAM_FALLBACK_OPTIONS option)
    {
    ...
    int
    RtcEngineBridge::switchCamera(CAMERA_DIRECTION direction)
    {
    ...
    int
    RtcEngineBridge::setDefaultAudioRouteToSpeakerphone(bool defaultToSpeaker)
    {
        LOG_JSON(SET_DEFAULT_AUDIO_ROUTE_SPEAKER_PHONE, "defaultToSpeaker", defaultToSpeaker);
        return mRtcEngine->setDefaultAudioRouteToSpeakerphone(defaultToSpeaker);
    }

    int
    RtcEngineBridge::setEnableSpeakerphone(bool speakerOn)
    {
    ...
    int
    RtcEngineBridge::enableInEarMonitoring(bool enabled)
    {
    ...
    int
    RtcEngineBridge::setInEarMonitoringVolume(int volume)
    {
    ...
    bool
    RtcEngineBridge::isSpeakerphoneEnabled()
    {
...
#if (defined(__APPLE__) && TARGET_OS_IOS)
    int
    RtcEngineBridge::setAudioSessionOperationRestriction(rtc::AUDIO_SESSION_OPERATION_RESTRICTION restriction)
    {
...
#if (defined(__APPLE__) && TARGET_OS_MAC && !TARGET_OS_IPHONE) || defined(_WIN32)
    int
    RtcEngineBridge::enableLoopbackRecording(bool enabled, const char* deviceName)
    {
        LOG_JSON(ENABLE_LOOP_BACK_RECORDING, "enabled", enabled, "deviceName", deviceName);
        return mRtcEngine->enableLoopbackRecording(enabled, deviceName);
    }

#if (defined(__APPLE__) && TARGET_OS_MAC && !TARGET_OS_IPHONE)
    int
    RtcEngineBridge::startScreenCaptureByDisplayId(unsigned int displayId,
        const rtc::Rectangle& regionRect,
        const rtc::ScreenCaptureParameters& captureParams)
    {
        LOG_JSON(START_SCREEN_CAPTURE_BY_DISPLAY_ID, displayId, regionRect, captureParams);
        return mRtcEngine->startScreenCaptureByDisplayId(displayId, regionRect, captureParams);
    }
#endif

#if defined(_WIN32)
    int
    RtcEngineBridge::startScreenCaptureByScreenRect(const rtc::Rectangle& screenRect,
        const rtc::Rectangle& regionRect,
        const rtc::ScreenCaptureParameters& captureParams)
    {
        LOG_JSON(START_SCREEN_CAPTURE_BY_SCREEN_RECT, screenRect, regionRect, captureParams);
        return mRtcEngine->startScreenCaptureByScreenRect(screenRect, regionRect, captureParams);
    }

    int
    RtcEngineBridge::startScreenCaptureByWindowId(rtc::view_t windowId,
        const rtc::Rectangle& regionRect,
        const rtc::ScreenCaptureParameters& captureParams)
    {
        LOG_JSON(START_SCREEN_CAPTURE_BY_WINDOW_ID, windowId, regionRect, captureParams);
        return mRtcEngine->startScreenCaptureByWindowId(windowId, regionRect, captureParams);
    }

    int
    RtcEngineBridge::setScreenCaptureContentHint(rtc::VideoContentHint contentHint)
    {
    ...
    int
    RtcEngineBridge::updateScreenCaptureParameters(const rtc::ScreenCaptureParameters& captureParams)
    {
    ...
    int
    RtcEngineBridge::updateScreenCaptureRegion(const rtc::Rectangle& regionRect)
    {
    ...
    int
    RtcEngineBridge::updateScreenCaptureRegion(const rtc::Rect* rect)
    {
//        LOG_JSON(UPDATE_SCREEN_CAPTURE_REGION, *rect);
    ...
    int
    RtcEngineBridge::stopScreenCapture()
    {
    ...
    RtcChannelBridge*
    RtcEngineBridge::createChannel(const char* channelId)
    {
        return new RtcChannelBridge(mRtcEngine, channelId);
    }

    IAudioDeviceManagerBridge*
    RtcEngineBridge::createAudioDeviceManager(DEVICE_TYPE device_type, ERROR_CODE& erroCode)
    {
        switch (device_type) {
    ...
    VideoDeviceManager*
    RtcEngineBridge::createVideoDeviceManager(int* errorCode)
    {
        ERROR_CODE code;
    ...
    void
    RtcEngineBridge::release(bool sync)
    {
    ...
    int
    RtcEngineBridge::setupLocalVideo(const VideoCanvas& canvas)
    {
    ...
    int
    RtcEngineBridge::setupRemoteVideo(const VideoCanvas& canvas)
    {
    ...
    int
    RtcEngineBridge::setLocalVideoMirrorMode(VIDEO_MIRROR_MODE_TYPE mirrorMode)
    {
    ...
    int
    RtcEngineBridge::enableDualStreamMode(bool enabled)
    {
    ...
    int
    RtcEngineBridge::getCallId(agora::util::AString& callId)
    {
    ...
    int
    RtcEngineBridge::rate(const char* callId, int rating, const char* description)
    {
        LOG_JSON(RATE, "callId", callId, "rating", rating, "description", description);
        return mRtcEngine->rate(callId, rating, description);
    }

    int
    RtcEngineBridge::complain(const char* callId, const char* description)
    {
    ...
    int
    RtcEngineBridge::enableLastmileTest()
    {
    ...
    int
    RtcEngineBridge::disableLastmileTest()
    {
    ...
    int
    RtcEngineBridge::startLastmileProbeTest(const LastmileProbeConfig& config)
    {
    ...
    int
    RtcEngineBridge::stopLastmileProbeTest()
    {
    ...
    int
    RtcEngineBridge::setEncryptionSecret(const char* secret)
    {
    ...
    int
    RtcEngineBridge::setEncryptionMode(const char* encryptionMode)
    {
    ...
    int
    RtcEngineBridge::registerPacketObserver(IPacketObserver* observer)
    {
        // TODO
        return 0;
    }
    ...
    int
    RtcEngineBridge::createDataStream(int* streamId, bool reliable, bool ordered)
    {
    ...
    int
    RtcEngineBridge::sendStreamMessage(int streamId, const char* data, size_t length)
    {
        LOG_JSON(SEND_STREAM_MESSAGE, "streamId", streamId, "length", (uint64_t)length);
        return mRtcEngine->sendStreamMessage(streamId, data, length);
    }

    int
    RtcEngineBridge::addPublishStreamUrl(const char* url, bool transcodingEnabled)
    {
        LOG_JSON(ADD_PUBLISH_STREAM_URL, "url", url, "transcodingEnabled", transcodingEnabled);
        return mRtcEngine->addPublishStreamUrl(url, transcodingEnabled);
    }

    int
    RtcEngineBridge::removePublishStreamUrl(const char* url)
    {
    ...
    int
    RtcEngineBridge::setLiveTranscoding(const LiveTranscoding& transcoding)
    {
    ...
    int
    RtcEngineBridge::addVideoWatermark(const RtcImage& watermark)
    {
    ...
    int
    RtcEngineBridge::addVideoWatermark(const char* watermarkUrl,
    ...
    int
    RtcEngineBridge::clearVideoWatermarks()
    {
    ...
    int
    RtcEngineBridge::setBeautyEffectOptions(bool enabled, BeautyOptions options)
    {
    ...
    int
    RtcEngineBridge::addInjectStreamUrl(const char* url, const InjectStreamConfig& config)
    {
    ...
    int
    RtcEngineBridge::startChannelMediaRelay(const ChannelMediaRelayConfiguration& configuration)
    {
    ...
    int
    RtcEngineBridge::updateChannelMediaRelay(const ChannelMediaRelayConfiguration& configuration)
    {
    ...
    int
    RtcEngineBridge::stopChannelMediaRelay()
    {
    ...
    int
    RtcEngineBridge::removeInjectStreamUrl(const char* url)
    {
    ...
    int
    RtcEngineBridge::registerMediaMetadataObserver(IMetadataObserver* observer, IMetadataObserver::METADATA_TYPE type, bool useSdkDefault)
    {
        LOG_JSON(REGISTER_MEDIA_META_DATA_OBSERVER, "type", type, "useSdkDefault", useSdkDefault);
        int ret;
        if (useSdkDefault) {
    ...
    int
    RtcEngineBridge::setParameters(const char* parameters)
    {
    ...
    int
    RtcEngineBridge::sendMetadata(rtc::IMetadataObserver::Metadata* metadata)
    {
    ...
    int
    RtcEngineBridge::setMaxMetadataSize(int size)
    {
    ...
    int
    RtcEngineBridge::pushAudioFrame(media::MEDIA_SOURCE_TYPE type,
    ...
    int
    RtcEngineBridge::pushAudioFrame(agora::media::IAudioFrameObserver::AudioFrame* frame)
    {
        if (!mediaEngine.get()) {
            mediaEngine.queryInterface(mRtcEngine, agora::AGORA_IID_MEDIA_ENGINE);
        }
    ...
    int
    RtcEngineBridge::pullAudioFrame(media::IAudioFrameObserver::AudioFrame* frame)
    {
        if (!mediaEngine.get()) {
            mediaEngine.queryInterface(mRtcEngine, agora::AGORA_IID_MEDIA_ENGINE);
        }
    ...
    int
    RtcEngineBridge::setExternalVideoSource(bool enable, bool useTexture)
    {
        if (!mediaEngine.get()) {
            mediaEngine.queryInterface(mRtcEngine, agora::AGORA_IID_MEDIA_ENGINE);
        }
    ...
    int
    RtcEngineBridge::pushVideoFrame(media::ExternalVideoFrame* frame)
    {
        if (!mediaEngine.get()) {
            mediaEngine.queryInterface(mRtcEngine, agora::AGORA_IID_MEDIA_ENGINE);
        }
    ...
    int
    RtcEngineBridge::enableEncryption(bool enabled, const agora::rtc::EncryptionConfig& config)
    {
