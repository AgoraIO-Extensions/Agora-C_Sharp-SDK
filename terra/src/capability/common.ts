...
//keep onLocalVideoStateChanged both in IRtcEngineEventHandler and IRtcEngineEventHandlerEx
...
export function processIRtcEngineEventHandler() {
    ...
    eventEx.methods.forEach(methodEx => {
        ...
        if (method && method.name != "onLocalVideoStateChanged") {
        ...
        if (methodEx.name == "onLocalVideoStateChanged") {
            methodEx.user_data = methodEx.user_data || {};
            methodEx.user_data.repeat = 2;
        }
    });
...
//Process IVideoFrameObserver's method has videoFrame parameter
export function processIVideoFrameObserver() {
    let clonedParseResult: ParseResult = global.clonedParseResult;
    const event = clonedParseResult.resolveNodeByName("agora::media::IVideoFrameObserver") as Clazz;

    event.methods.forEach(method => {
        method.user_data = method.user_data || {};
        const hasVideoFrame = method.parameters.some(p => {
            return p.name == "videoFrame";
        });
        method.user_data.hasVideoFrame = hasVideoFrame;
    });
...
export function processCppConstructor(clazzName: string, fullFilePath: string): CppConstructor[] {
    ...
    if (array) {
        for (let e of array) {
            ...
            let leftSidePos = e.indexOf("{");
            let initializePos = e.indexOf(":", e.indexOf(")"));
            if (initializePos != -1 && initializePos < leftSidePos) {
