...
namespace Agora.Rtc
{
    ...
    internal class H265TranscoderImpl
    {
        ...
        public int EnableTranscode(string token, string channel, uint uid)
        {
            ...
            var nRet = AgoraRtcNative.CallIrisApiWithArgs(_irisApiEngine, "H265Transcoder_enableTranscode",
        ...
        public int QueryChannel(string token, string channel, uint uid)
        {
            ...
            var json = AgoraJson.ToJson(_param);
            var nRet = AgoraRtcNative.CallIrisApiWithArgs(_irisApiEngine, "H265Transcoder_queryChannel",
                json, (UInt32)json.Length,
                IntPtr.Zero, 0,
                ref _apiParam);
            var result = nRet != 0 ? nRet : (int)AgoraJson.GetData<int>(_apiParam.Result, "result");
        ...
        public int TriggerTranscode(string token, string channel, uint uid)
        {
            ...
            var json = AgoraJson.ToJson(_param);
            var nRet = AgoraRtcNative.CallIrisApiWithArgs(_irisApiEngine, "H265Transcoder_triggerTranscode",
                json, (UInt32)json.Length,
                IntPtr.Zero, 0,
                ref _apiParam);
            var result = nRet != 0 ? nRet : (int)AgoraJson.GetData<int>(_apiParam.Result, "result");
